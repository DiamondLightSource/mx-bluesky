...
2. On this branch pin the latest release of dodal, and nexgen if necessary. (e.g "dls-dodal == 1.63.0")
3. Run a ``uv sync`` to update the ``uv.lock`` file
4. push to GitHub
5. Make sure the CI is passing for this new pre-release branch.
6. Go `here <https://github.com/DiamondLightSource/mx-bluesky/releases/new>`_.
7. Select Choose a new tag and type the version of the release, then select the branch created in step 1 as the target.
8. Click on Generate release notes. This will create a starting set of release notes based on PR titles since the last release.
9. You should now manually go through each line on the release notes and read it from the perspective of a beamline scientist. It should be clear from each what the change means to the beamline and should have links to easily find further info.
