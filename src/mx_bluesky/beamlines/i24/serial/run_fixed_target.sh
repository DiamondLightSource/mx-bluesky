...
blueapi -c "${current}/blueapi_config.yaml" controller run setup_collection_logs '{"expt":"Serial Fixed"}' -i $INSTRUMENT_SESSION
...
blueapi -c "${current}/blueapi_config.yaml" controller run clean_up_log_config_at_end -i $INSTRUMENT_SESSION
