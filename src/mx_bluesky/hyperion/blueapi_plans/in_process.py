"""
This module contains the bluesky plan entry points for use with hyperion.
hyperion-blueapi loads this module via blueapi.py which contains module-load initialisation
hooks that are run when blueapi loads the plan module, which are not needed by the
in-process runner and so not defined here.
