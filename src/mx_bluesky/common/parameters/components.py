...
class BaseParameters(BaseModel):
    ...
    def _validate_version(cls, version_str: str):
        """Assert that parsed in version is within bounds of current version of mx-bluesky.
        See Hyperion implementation for example"""
        pass
...
class WithVisit(BaseModel):
    ...
    zocalo_environment: str | None = Field(
        default=CONST.ZOCALO_ENV
    )  # None if Zocalo not used in experiment
...
class DiffractionExperiment(
    BaseParameters, WithSnapshot, WithOptionalEnergyChange, WithVisit
):
