...
def eiger_arm(device_composite: FlyScanEssentialDevices[Any, EigerDetector],
              detector_params: DetectorParams,
              group: str) -> MsgGenerator:
    yield from bps.stage(device_composite.detector, group=group)


def eiger_disarm(device_composite: FlyScanEssentialDevices[Any, EigerDetector]) -> MsgGenerator:
...
def eiger_zocalo_hw_read_mapper(doc: Event) -> ZocaloHWReadPayload:
    return ZocaloHWReadPayload(
        file_name=doc["data"]["eiger_odin_file_writer_id"]
    )
