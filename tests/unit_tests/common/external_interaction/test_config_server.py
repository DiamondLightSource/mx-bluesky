...
from mx_bluesky.common.parameters.constants import FeatureFlagSources, OavConstants
...
def test_get_feature_flags_cache():
    ...
    server._time_since_feature_get = time()
...
def test_clear_cache():
    ...
    server._cached_features = HyperionFeatureFlags(**features_dict)
    server._cached_oav_config = {"test_key": "test_val"}
    server.clear_cache()
    assert server._cached_features is None
    assert server._cached_oav_config is None
