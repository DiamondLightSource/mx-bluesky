...
def test_begin_deposition(
    ...
    create_dcg_request = mock_ispyb_conn.calls_for(DCG_RE)[0].request
    assert DCG_RE.match(create_dcg_request.url)[2] == TEST_PROPOSAL_REF
    assert int(DCG_RE.match(create_dcg_request.url)[3]) == TEST_VISIT_NUMBER
