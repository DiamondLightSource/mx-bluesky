...
class TestXrayCentreISPyBCallback:
    ...
    def test_activity_gated_event_oav_snapshot_triggered(
        ...
        assert (
            int(mock_ispyb_conn.match(update_grid_requests[0], GRID_RE, 2))
            == TEST_DATA_COLLECTION_IDS[0]
        )
        assert (
            int(mock_ispyb_conn.match(update_grid_requests[1], GRID_RE, 2))
            == TEST_DATA_COLLECTION_IDS[1]
        )
        assert json.loads(update_grid_requests[0].body) == {
            "dx": 0.1264,
            "dy": 0.1264,
            "stepsX": 40,
            "stepsY": 20,
        ...
        assert json.loads(update_grid_requests[1].body) == {
            "dx": 0.1264,
            "dy": 0.1264,
            "stepsX": 40,
            "stepsY": 10,
