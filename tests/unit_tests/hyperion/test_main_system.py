...
def test_when_context_created_then_contains_expected_number_of_plans(
    ...
    with patch.dict(
        ...
        with patch(
            ...
            context = setup_context(wait_for_connection=False, dev_mode=dev_mode)
            mock_with_dodal_module.assert_called_once_with(
                ANY, wait_for_connection=False, mock=dev_mode
            )
...
def test_create_app_passes_through_dev_mode(
    ...
    create_app({"TESTING": True}, mock_run_engine, False, dev_mode=dev_mode)

    mock_setup_context.assert_called_once_with(
        wait_for_connection=True, dev_mode=dev_mode
    )
