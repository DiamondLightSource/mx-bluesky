...
@pytest.fixture()
def diffraction_extended_devices(
...
def test_given_shutter_open_fails_then_eiger_disarmed_and_correct_exception_returned(
    ...
    diffraction_extended_devices.detector_motion.z.set = MagicMock(
        side_effect=MyTestError()
    )
...
def test_given_detector_move_fails_then_eiger_disarmed_and_correct_exception_returned(
    ...
    diffraction_extended_devices.detector_motion.shutter.set = MagicMock(
        side_effect=MyTestError
    )
    ...
    get_mock_put(
        diffraction_extended_devices.detector_motion.z.user_setpoint
    ).assert_called_once()
    diffraction_extended_devices.detector.disarm_detector.assert_called_once()
